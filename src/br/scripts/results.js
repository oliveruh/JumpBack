document.addEventListener('DOMContentLoaded', function () {
    ...
    function populateStatsTab(issueData, allSeriesData) {
        ...
        if (!issueData || !issueData.table_of_contents || !allSeriesData) {
            // TRADUÇÃO AQUI
            elements.statsTabPanel.innerHTML = '<h2>Estatísticas</h2><p>As estatísticas para esta edição não estão disponíveis no momento.</p>';
            return;
        }

        // TRADUÇÃO AQUI
        let statsHTML = '<h2>Estatísticas da Edição</h2>';

        // --- Top Three Series (from Table of Contents) ---
        // TRADUÇÃO AQUI
        statsHTML += '<h3>Séries em Destaque Nesta Edição:</h3>';
        ...
        if (tocSeries && tocSeries.length > 0) {
            ...
            tocSeries.slice(0, 3).forEach(tocEntry => {
                let seriesTitle = tocEntry.title;
                statsHTML += `<li><strong>${seriesTitle}</strong></li>`;
            });
            statsHTML += '</ol>';
        } else {
            // TRADUÇÃO AQUI
            statsHTML += '<p>Os dados do sumário não estão disponíveis para esta edição.</p>';
        }
        statsHTML += '<hr class="my-3 border-gray-300">';

        // --- Most Recurring Categories with Chart ---
        // TRADUÇÃO AQUI
        statsHTML += '<h3>Tags mais comuns nesta edição:</h3>';
        ...
        if (seriesInIssueDetails.length > 0) {
            ...
            if (sortedCategories.length > 0) {
                statsHTML += '<div style="max-width: 400px; margin: 10px auto;"><canvas id="tagChart"></canvas></div>';
                ...
                Object.entries(categoryCounts).sort(([, a], [, b]) => b - a).slice(0, 5).forEach(([category, count]) => {
                    // TRADUÇÃO AQUI
                    statsHTML += `<li>${category} (aparece em ${count} séries)</li>`;
                });
                statsHTML += '</ul>';

            } else {
                // TRADUÇÃO AQUI
                statsHTML += '<p>Não há dados de categoria disponíveis para as séries desta edição.</p>';
            }
        } else {
            // TRADUÇÃO AQUI
            statsHTML += '<p>Nenhum dado de série foi encontrado para gerar estatísticas para esta edição.</p>';
        }
                    ...
                    state.genreChartInstance = new Chart(tagChartCanvas, { // Use new canvas id
                        ...
                        options: {
                            ...
                            plugins: {
                                ...
                                title: {
                                    display: true,
                                    text: 'Top 5 TAGs Associadas' // Updated chart title
                                }
                            }
                        }
                    });
    ...
    function populateHiddenGemTab(issueData, allSeriesData) {
        ...
        let gemHTML = `<h2>HIDDEN GEM da Edição</h2>`; // Changed heading
        if (hiddenGem) {
            ...
            if (hiddenGem.genres && hiddenGem.genres.length > 0) {
                gemHTML += `<p><strong>Gêneros:</strong> ${hiddenGem.genres.join(', ')}</p>`;
            }
            ...
            if (hiddenGem.mu_url) {
                gemHTML += `<p class="mt-2"><a href="${hiddenGem.mu_url}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline">Mais informações no MangaUpdates</a></p>`;
            }
    ...
    function populateResultsPage(issueData, allSeriesData) { // Added allSeriesData
        ...
        if (elements.coverDate) {
            ...
            elements.coverDate.textContent = dateObj.toLocaleDateString('pt-BR', {
    ...
    async function displayHoroscope(birthDateString) {
        ...
        if (userSign) {
            ...
            html += `<p style="margin-bottom: 5px;"><strong>Arquético:</strong> ${userSign.shonen_archetype || 'N/A'}</p>`;
            html += `<p style="margin-bottom: 5px;"><strong>Personagem:</strong> ${userSign.character_name || 'N/A'} <em>(${userSign.series || 'N/A'} - ${userSign.era || 'N/A'})</em></p>`;
            html += `<p style="clear:right; margin-bottom: 8px;"><em>${userSign.description || 'No description available.'}</em></p>`;
            html += `<p><strong>Poder:</strong> ${userSign.shonen_power_type || 'N/A'}</p>`;
            html += `</div>`;
